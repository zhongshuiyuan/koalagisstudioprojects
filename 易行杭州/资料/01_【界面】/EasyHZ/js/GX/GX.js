...
var GX = Fns.Create({
	...
	anime: function(styles, duration, easing, callback) {
		if(!this.isRunning) {
			...
			this.changFn=(typeof callback == 'object') ? callback.change : false;//add by cl
		...
		if (elapsedTime < this.duration) {
			for(var style in this.styles) {
				...
				if(typeof starts != 'object') {
					...
					if(this.changFn)    this.changFn(style,ease(elapsedTime, starts, changes, this.duration));//add by cl
				}
				else { // colors
