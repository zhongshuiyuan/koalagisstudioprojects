...
function SGWorld(divID, clientOnInitFinishedHandler, referrerId)
{
    ...
    try
    {
        if (divID != undefined && divID != "")
        {
            ...
            try {TE3DWindow.text = "@1";}catch(e){if (e.description=="@1") this.bV510orGreater=true;}
        }
        else
        {
            // If we are here, make sure we are not opening a new tab in a browser which already has TE embeded in another tab (IE7 -> causing crash)            
        }
...
function _SGPopup_process(thePopup, autoRemoveOldPopup)
{
    ...
    {    
        ...
        if (popup.hideContentUntilLoaded == true && msgType == 1)
            content = sgSGAPIURL + "loading.html?showimage=0&framesrc=" + escape(content);
